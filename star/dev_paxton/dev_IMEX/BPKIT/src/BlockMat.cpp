    ...
    for (i=0; i<nrow; i++)
    {
		...
		for (ii=0; ii<neqr; ii++) // scalar rows
		{
		    ...
		    ia_[ii+1] = kk;
                ...
                // cerr << (CSRMat) p[k] << std::endl << std::endl;
...
{
    std::ostream::fmtflags fmt = os.flags();
    ...
    for (int i=0; i<mat.numrow(); i++)
    {
       ...
       {   
          os << std::setw(14) << std::right << i+1;
          os << std::setw(14) << std::right << mat.col_ind(j)+1;
          // print first entry of block in block matrix
          os << std::scientific << std::setw(20) << std::right 
                                                 << *(mat.val(j).Data());
          os << std::endl;
       }
    }
