...
#if DEBUG
        if (iwk[i] < 0)
           cerr << "APINV0: no diag in col " << i << std::endl;
#endif
...
#if DEBUG
        if (iwk[i] < 0)
           cerr << "APINV_BANDED: no diag in col " << i << std::endl;
#endif
...
void CSRMat::Print(std::ostream& os) const
{
        assert (a != NULL);
...
std::ostream& operator << (std::ostream& os, const CSRMat& mat)
{
        assert (mat.a != NULL);
