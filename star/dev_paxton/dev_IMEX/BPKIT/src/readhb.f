...
c-----------------------------------------------------------------------
c     Simplified function for reading HB matrix from stdin.
c     if nrhs.ne.0, then rhs has space for 3*n elements.
c     Warning: nnz and nrhs may be changed on output.
c-----------------------------------------------------------------------
      subroutine readhb(filename, flen, n, nnz, nrhs, a, ja, ia, 
     *    rhs, guess, exact, temp)
      implicit none
      character*80 filename
      integer flen, n, nnz, nrhs, ja(*), ia(*)
      real*8  a(*), rhs(*), guess(*), exact(*), temp(*)
c-----------------------------------------------------------------------
      integer   job, ncol, nrow, ierr
      character guesol*2, title*72, key*8, type*3

      open(9,file=filename(1:flen),status='OLD')
      ...
      call bpreadmt(n, nnz, job, 9, a, ja, ia, temp, nrhs,
     *     guesol, nrow, ncol, nnz, title, key, type, ierr)
      close(9)
      if (ierr .ne. 0) then
         print *,'readhb: could not read matrix, ierr = ', ierr
         stop
      endif
...
!-----------------------------------------------------------------------
!
! see the file ilut.f for Copyright, Licensing, and Disclaimer.
!
c-----------------------------------------------------------------------
	subroutine bpreadmt (nmax,nzmax,job,iounit,a,ja,ia,rhs,nrhs,
     *                     guesol,nrow,ncol,nnz,title,key,type,ierr)
c-----------------------------------------------------------------------
