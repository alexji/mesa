      ...
      const double *b, int ldb, double *c, int ldc)
{
  ...
  if (unitd != 1) {
    std::cerr << "unitd != 1 not implemented" << std::endl;
    exit(1);
  }
...
{
  if (descra[0] != 0) {
    std::cerr << "Must have general matrix" << std::endl;
    exit(1);
  }

  if (beta != 0.0) {
    std::cerr << "beta != 0 not implemented" << std::endl;
    exit(1);
  }
  ...
  if (transa == 0) {
    ...
    CompCol_LowerUnitSolve_float(m, n, unitd, dv, alpha, val, indx, pntr,
        ...
        std::cerr << "Bad descra[2]" << std::endl;
    exit(1);
      }
    } else if (descra[1] == 2) {
      if (descra[2] == 0)
    ...
    exit(1);
      ...
      std::cerr << "Bad descra[1]" << std::endl;
      exit(1);
    }
  } else {
    std::cerr << "Bad transa" << std::endl;
    exit(1);
  }
}
