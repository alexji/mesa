...
! ***********************************************************************
      ...
      interface 
         function f_dispatch_c( &
               iop, &
               lrpar, rpar, &
               lipar, ipar, &
               lrdat, rdat, &
               lidat, idat) &
               bind(C,name='dispatch')
            use, intrinsic :: ISO_C_BINDING, only: C_INT, C_DOUBLE
            integer(C_INT) :: f_dispatch_c
            integer(C_INT) :: iop,  lrpar, lipar, ipar, lrdat, lidat, idat
            real(C_double) :: rpar, rdat
         end function f_dispatch_c
      end interface
      ...
      subroutine do_hydro(s, ierr)
         ...
         if (ierr /= 0) stop 'do_hydro start error'
         ! loop doing steps
         nsteps_taken = 0
         time = 0d0
         max_calls = 10000
         ...
         do i = 1, max_calls
            ...
            if (s% job% pgstar_flag) then
               ...
               if (ierr /= 0) stop 'do_hydro update_pgstar_plots error'
            end if
            if (final_step) exit
         end do
         ! finish hydro
