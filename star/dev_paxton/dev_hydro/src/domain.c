...
void setICparams( struct domain * );         // initial conditions: bill.c, hse.c, ...
void setHydroParams( struct domain * );      // euler.c
void setRiemannParams( struct domain * );    // riemann.c
void setGravityParams( struct domain * );    // gravity.c
...
void possiblyOutput( struct domain * theDomain , int override ){
   ...
   double nzpt = theDomain->N_rpt;
   double Nchk = theDomain->N_chk;
   int LogOut = theDomain->theParList.Out_LogTime;
   int n0;

   n0 = (int)( t*nzpt/t_fin );
   if( LogOut ) n0 = (int)( nzpt*log(t/t_min)/log(t_fin/t_min) );
   ...
   if( (theDomain->nchk < n0 && Nchk>0) || override ){
      ...
      char filename[256];
      if( !override ){
         if(0==0) printf("Creating Checkpoint #%04d...\n",n0);
         sprintf(filename,"photos/checkpoint_%04d",n0);
         output( theDomain , filename );
      }else{
         if(0==0) printf("Creating Final Checkpoint...\n");
         output( theDomain , "photos/checkpoint_final" );
      }
   }
