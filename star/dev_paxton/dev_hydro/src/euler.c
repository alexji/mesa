...
void prim2cons( double * prim , double * cons , double GMr , double dV ){
   ...
   double rhoe = Pp/(gam-1.); // internal energy density

   double egrav = -rho*GMr; // potential energy density

   cons[DDD] = rho*dV; // cell mass
   cons[SRR] = rho*vr*dV; // cell momentum
   cons[TAU] = (.5*rho*v2 + rhoe + egrav)*dV; // cell total energy
...
void cons2prim( double * cons , double * prim , double GMr , double dV ){
   ...
   double rho = cons[DDD]/dV; // mass density
   double Sr  = cons[SRR]/dV; // momentum density
   double E   = cons[TAU]/dV; // total energy density

   double egrav = -rho*GMr; // potential energy density

   double vr = Sr/rho;
   double v2 = vr*vr;
   double rhoe = E - .5*rho*v2 - egrav; // internal energy density
   double gam = GAMMA_LAW;
